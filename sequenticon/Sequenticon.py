...
def sequenticon(sequence, output_format="png", size=60, output_path=None):
    ...
    generator = pydenticon.Generator(
        SETTINGS["rows"], SETTINGS["columns"], foreground=SETTINGS["color_palette"]
    )
    ...
    data = {"png": img, "base64": base_64, "html_image": html_image}[output_format]
...
def sequenticon_batch(sequences, output_format="png", size=60, output_path=None):
...
def sequenticon_batch_pdf(sequences, target=None, title="Sequenticons batch"):
    ...
    html = pug_to_html(
        PUG_TEMPLATE,
        sidebar_text="Generated on %s by Sequenticon version %s" % (now, __version__),
